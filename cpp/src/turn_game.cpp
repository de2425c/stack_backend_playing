...
std::vector<TurnAction> legal_actions(const State& state,
                                      int base_pot,
                                      int stack,
...
State next_state(const State& state, const TurnAction& action) {
    State next = state;
    int player = state.player;
...
TurnGame::TurnGame(const TurnConfig& config) {
    if (config.board_cards.size() != 4) {
        throw std::runtime_error("Turn game requires 4 board cards");
    }
    base_pot = config.pot;
    stack = config.stack;
...
std::vector<TurnHand> TurnGame::build_hands(const std::vector<std::array<int, 2>>& hole_cards,
    ...
    for (std::size_t i = 0; i < cards.size(); ++i) {
        ...
        // Note: Unlike river, we don't compute strength here since it depends on river card.
        built.push_back(TurnHand{hand_cards, weight});
    }
    return built;
}

TurnTree TurnGame::build_tree() const {
    TurnTree tree;
    ...
    tree.nodes.push_back(TurnTreeNode{root_state.player, root_state.terminal_winner, root_state.contrib0,
    ...
    while (index < states.size()) {
        ...
        for (std::size_t a_idx = 0; a_idx < actions.size(); ++a_idx) {
            ...
            tree.nodes.push_back(TurnTreeNode{child_state.player, child_state.terminal_winner, child_state.contrib0,
