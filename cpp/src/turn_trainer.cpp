...
TurnTrainer::TurnTrainer(const TurnGame& game, const TurnTree& tree, TurnAlgorithm algo, TurnDcfrParams dcfr)
    : game_(game), tree_(tree), evaluator_(game.hands, game.board_cards), algo_(algo), dcfr_(dcfr) {
    ...
    for (std::size_t i = 0; i < tree_.nodes.size(); ++i) {
        const TurnTreeNode& node = tree_.nodes[i];
        if (node.player < 0) {
            continue;
        }
        ...
        info.regret.assign(total, TurnCFRScalar(0));
        info.strategy_sum.assign(total, TurnCFRScalar(0));
    }
...
bool TurnTrainer::has_infoset(int node_id) const {
...
int TurnTrainer::infoset_action_count(int node_id) const {
...
int TurnTrainer::infoset_hand_count(int node_id) const {
    if (!has_infoset(node_id)) {
        return 0;
    }
...
void TurnTrainer::average_strategy(int node_id, std::vector<double>& out) const {
    if (!has_infoset(node_id)) {
        out.clear();
        return;
    }
...
void TurnTrainer::compute_strategy(const InfoSet& info, double* out_strategy) const {
    // Regret-matching over positive regrets per hand.
    int hand_count = info.hand_count;
    int action_count = info.action_count;
    const TurnCFRScalar* regret = info.regret.data();
...
void TurnTrainer::compute_avg_strategy(const InfoSet& info, double* out_strategy) const {
    // Normalize cumulative strategy sums to get the average strategy.
    int hand_count = info.hand_count;
    int action_count = info.action_count;
    const TurnCFRScalar* strategy_sum = info.strategy_sum.data();
    for (int h = 0; h < hand_count; ++h) {
        double normalizing = 0.0;
        int offset = h * action_count;
        for (int a = 0; a < action_count; ++a) {
...
void TurnTrainer::apply_dcfr_discount(InfoSet& info, double pos_scale, double neg_scale, double strat_scale) const {
    // Apply DCFR per-iteration decay to keep regrets/averages bounded.
    for (TurnCFRScalar& regret : info.regret) {
        if (regret > TurnCFRScalar(0)) {
            regret = static_cast<TurnCFRScalar>(static_cast<double>(regret) * pos_scale);
        } else if (regret < TurnCFRScalar(0)) {
            regret = static_cast<TurnCFRScalar>(static_cast<double>(regret) * neg_scale);
        }
    }
    for (TurnCFRScalar& value : info.strategy_sum) {
        value = static_cast<TurnCFRScalar>(static_cast<double>(value) * strat_scale);
    }
}

const double* TurnTrainer::traverse(int node_id,
                                    int update_player,
                                    const double* reach_p,
                                    const double* reach_opp,
                                    int depth) {
    ScratchFrame& frame = scratch_[depth];
    const TurnTreeNode& node = tree_.nodes[node_id];
    ...
    if (node.player == -1) {
        ...
        if (node.terminal_winner >= 0) {
            ...
            evaluator_.showdown_values(update_player, reach_opp, pot, contrib, frame.values.data());
        }
    ...
    TurnCFRScalar* regret = info.regret.data();
    for (int h = 0; h < update_hands; ++h) {
        int offset = h * action_count;
        ...
        for (int a = 0; a < action_count; ++a) {
            ...
            if (algo_ == TurnAlgorithm::CFR_PLUS) {
                regret[offset + a] = static_cast<TurnCFRScalar>(updated > 0.0 ? updated : 0.0);
            } else {
                regret[offset + a] = static_cast<TurnCFRScalar>(updated);
            }
        }
    }

    TurnCFRScalar* strategy_sum = info.strategy_sum.data();
    for (int h = 0; h < update_hands; ++h) {
        ...
        int offset = h * action_count;
        for (int a = 0; a < action_count; ++a) {
            strategy_sum[offset + a] = static_cast<TurnCFRScalar>(static_cast<double>(strategy_sum[offset + a]) +
...
const double* TurnTrainer::best_response(int node_id,
                                         int target_player,
                                         const double* reach_opp,
                                         int depth) const {
    ScratchFrame& frame = scratch_[depth];
    ...
    if (node.player == -1) {
        ...
        if (node.terminal_winner >= 0) {
            ...
            evaluator_.showdown_values(target_player, reach_opp, pot, contrib, frame.values.data());
        }
        return frame.values.data();
    }
...
double TurnTrainer::best_response_value(int target_player) const {
    int target_hands = num_hands_[target_player];
